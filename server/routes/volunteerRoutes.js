...
module.exports = (
  ...
  // VOLUNTEER PROFILE
  router.get("/volunteer/profile", authenticate, authorizeVolunteer, async (req, res) => {
    try {
  ...
  router.post("/availabilities", authenticate, authorizeVolunteer, async (req, res) => {
    try {
      const availabilities = req.body;
      const volunteerId = req.user.userId;
  
      if (!Array.isArray(availabilities)) {
        return res.status(400).json({ error: "Invalid input: Availabilities must be an array." });
      }
      ...
      if (lastUpdated) {
        ...
        if (daysSinceUpdate < 30) {
          return res.status(403).json({
            error: `Vous ne pouvez modifier vos disponibilités qu'une fois tous les 30 jours. Prochaine mise à jour possible dans ${30 - daysSinceUpdate} jours.`,
          });
        }
      }
      ...
      if (parseInt(ongoingReservations.rows[0].count) > 0) {
        return res.status(403).json({
          error: "Vous ne pouvez pas modifier vos disponibilités tant que vous avez des réservations en cours (en attente ou acceptées).",
        });
      }
  
      await pool.query("DELETE FROM availabilities WHERE user_id = $1", [volunteerId]);
  
      for (const availability of availabilities) {
        const { dayOfWeek, startTime, endTime } = availability;
        if (
          typeof dayOfWeek !== "number" || dayOfWeek < 1 || dayOfWeek > 7 ||
          typeof startTime !== "string" || !isValidTime(startTime) ||
  ...
  router.get("/volunteer/villages-covered", authenticate, authorizeVolunteer, async (req, res) => {
    try {
      const volunteerId = req.user.userId;
      const volunteer = await pool.query(
  ...
  router.put("/volunteer/villages-covered", authenticate, authorizeVolunteer, async (req, res) => {
    try {
      const volunteerId = req.user.userId;
      const { villagesCovered } = req.body;
  
      if (!Array.isArray(villagesCovered)) {
        return res.status(400).json({ error: "Villages covered must be an array." });
      }
      ...
      if (lastUpdated) {
        ...
        if (daysSinceUpdate < 30) {
          return res.status(403).json({
            error: `Vous ne pouvez modifier vos communes qu'une fois tous les 30 jours. Prochaine mise à jour possible dans ${30 - daysSinceUpdate} jours.`,
          });
        }
      }
      ...
      if (parseInt(ongoingReservations.rows[0].count) > 0) {
        return res.status(403).json({
          error: "Vous ne pouvez pas modifier vos communes tant que vous avez des réservations en cours (en attente ou acceptées).",
        });
      }
      ...
      const removedVillages = currentVillages.filter((v) => !villagesCovered.includes(v));
      if (removedVillages.length > 0) {
        connectedClients.forEach((client) => {
          if (client.readyState === WebSocket.OPEN && removedVillages.includes(client.village)) {
            client.send(
              JSON.stringify({
                type: "village_update",
                message: `Le bénévole ${req.user.username} ne couvre plus ${removedVillages.join(", ")}.`,
              })
            );
          }
        });
